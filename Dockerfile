...
RUN echo "0 2 */3 * * cd /app/cinema_scrapy && scrapy crawl movie >> /var/log/cron.log 2>&1" > /etc/cron.d/cinema-cron \
    && echo "30 2 */3 * * cd /app/cinema_scrapy && scrapy crawl tvshow >> /var/log/cron.log 2>&1" >> /etc/cron.d/cinema-cron \
